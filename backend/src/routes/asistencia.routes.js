...
router.get('/docente/:id', autenticar, async (req, res) => {
  try {
    const ingresos = await pool.query(
      'SELECT fecha, hora_registro, estado FROM registros_ingreso_institucional WHERE docente_id = $1 ORDER BY fecha DESC LIMIT 30',
      [req.params.id]
    );
    const cursos = await pool.query(
      `SELECT rac.fecha, rac.hora_registro, rac.estado, c.nombre AS curso, hc.aula
       ...
       ORDER BY rac.fecha DESC LIMIT 30`,
      [req.params.id]
    );
