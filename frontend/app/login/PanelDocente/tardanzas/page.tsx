...
export default function ConsultaTardanzasPage() {
    ...
    if (storedUser && token) {
      try {
        ...
        fetch(`/api/asistencia/docente/${parsedUser.id}`, { headers: { Authorization: `Bearer ${token}` } })
          .then((res) => res.json())
          .then((data) => {
            const arr: ConsolidatedTardanza[] = [];
  ...
  // --- Filtering ---
  const filteredTardanzas = tardanzas.filter((m) => {
  ...
  const uniqueCourseNames = Array.from(new Set(tardanzas.filter(m => m.tipo === "Curso").map(m => m.curso)));

  // --- Stats Calculations ---
  const countInstitucional = filteredTardanzas.filter(m => m.tipo === "Institucional").length;
  const countCursos = filteredTardanzas.filter(m => m.tipo === "Curso").length;
  
  const currentMonth = new Date().getMonth();
  const currentYear = new Date().getFullYear();
  const tardanzasEsteMes = tardanzas.filter(m => {
  ...
  filteredTardanzas.filter(m => m.tipo === "Curso").forEach(m => {
    courseFreq[m.curso] = (courseFreq[m.curso] || 0) + 1;
  });
  let mostFrequentCourse = "No hay registros";
  let maxCourseFreq = 0;
  ...
  const lastTardanza = filteredTardanzas.length > 0 ? formatDateLong(filteredTardanzas[0].fecha, filteredTardanzas[0].hora) : "Sin registros recientes";

  // Frequency by day
  const freqByDay = [0, 0, 0, 0, 0, 0, 0]; // Sun, Mon, Tue, Wed, Thu, Fri, Sat -> Wait, UI shows Lun to Dom.
  const uiDaysOrder = [1, 2, 3, 4, 5, 6, 0]; // Lun(1), Mar(2), Mie(3), Jue(4), Vie(5), Sab(6), Dom(0)
  const freqUiByDay = [0, 0, 0, 0, 0, 0, 0];

  filteredTardanzas.forEach(m => {
  ...
  if (loading) {
    return <div className="p-8 text-center text-slate-500 animate-pulse font-semibold">Cargando tardanzas...</div>;
  }
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 bg-slate-50 min-h-screen">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">
              Consulta de tardanzas
            </h1>
            <span className="bg-orange-50 text-orange-600 text-[10px] font-black px-2.5 py-1 rounded-full border border-orange-200 shadow-sm flex items-center gap-1.5">
               <span className="w-1.5 h-1.5 rounded-full bg-orange-500"></span>
               Solo lectura
            </span>
          </div>
          <p className="mt-1.5 text-sm font-semibold text-slate-500 max-w-2xl leading-relaxed">
            Revise las marcaciones de {userName} que el servidor clasifico como tardanza, tanto institucionales como academicas.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden">
          <CardContent className="p-5 flex items-start gap-4">
            ...
            <div>
              <p className="text-[11px] font-extrabold text-slate-800 mb-0.5">Tardanzas filtradas</p>
              <h3 className="text-3xl font-black text-amber-500 leading-none mb-1.5">{String(filteredTardanzas.length).padStart(2,'0')}</h3>
        ...
        <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden">
          <CardContent className="p-5 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-500 leading-snug">Marcaciones tardias de acceso general.</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden">
          <CardContent className="p-5 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-500 leading-snug">Sesiones academicas registradas como tardanza.</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden">
          <CardContent className="p-5 flex items-start gap-4">
            <div className="h-10 w-10 shrink-0 bg-rose-50 rounded-xl border border-rose-100 flex items-center justify-center text-rose-500">
            ...
            <div>
              <p className="text-[11px] font-extrabold text-slate-800 mb-0.5">Tardanzas este mes</p>
              <h3 className="text-3xl font-black text-rose-500 leading-none mb-1.5">{String(tardanzasEsteMes).padStart(2,'0')}</h3>
              <p className="text-[10px] font-bold text-slate-500 leading-snug">Conteo mensual dentro de los registros disponibles.</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden">
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-6">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-extrabold text-slate-800 ml-1">Tipo de tardanza</label>
          ...
          <div className="bg-amber-50/50 border border-amber-200 rounded-xl p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <p className="text-[10px] font-bold text-amber-900 leading-relaxed max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden flex flex-col">
          <div className="p-6 border-b border-slate-200">
            <div className="flex items-center gap-2 mb-1">
              <p className="text-[9px] font-extrabold text-orange-600 uppercase tracking-widest">Tendencia</p>
              <p className="text-[9px] font-extrabold text-orange-600 uppercase tracking-widest">Temporal</p>
            </div>
            <h2 className="text-xl font-black text-slate-900">Tardanzas por mes</h2>
          ...
          <CardContent className="p-8 flex-1 flex flex-col items-center justify-end h-64">
            {filteredTardanzas.length === 0 ? (
              <p className="text-sm font-semibold text-slate-400 my-auto">Sin datos en el periodo</p>
            ) : (
              <div className="flex items-end h-full gap-4 w-full px-4 justify-center">
                {/* Mock single bar since we just have total count for demo. In a real chart, group by month */}
                <div className="flex flex-col items-center">
                  <span className="text-[11px] font-black text-orange-600 mb-2">{filteredTardanzas.length}</span>
                  <div className="w-12 bg-orange-500 rounded-t-lg" style={{ height: '140px' }}></div>
        ...
        <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden flex flex-col">
          <div className="p-6 border-b border-slate-200">
            <div className="flex items-center gap-2 mb-1">
              <p className="text-[9px] font-extrabold text-orange-600 uppercase tracking-widest">Resumen</p>
              <p className="text-[9px] font-extrabold text-orange-600 uppercase tracking-widest">Del filtro</p>
            </div>
          ...
          <CardContent className="p-6 flex-1 flex flex-col justify-between">
             <div className="mb-6">
                <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1">Ultima tardanza</p>
                <h4 className="text-sm font-black text-slate-900">{lastTardanza}</h4>
             </div>
             <div className="mb-6">
             ...
             <div>
                ...
                <h4 className="text-sm font-black text-slate-900">{tardanzas.length} tardanza(s) devuelta(s) por el servidor</h4>
             </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden">
        <div className="p-6 border-b border-slate-200">
          <div className="flex items-center gap-2 mb-1">
            <p className="text-[9px] font-extrabold text-orange-600 uppercase tracking-widest">Distribucion</p>
            <p className="text-[9px] font-extrabold text-orange-600 uppercase tracking-widest">Semanal</p>
          </div>
          <h2 className="text-xl font-black text-slate-900">Frecuencia por dia</h2>
          <p className="text-xs font-semibold text-slate-500 mt-0.5">Comparacion de tardanzas filtradas segun el dia de la semana.</p>
        </div>
              ...
              {["Lun", "Mar", "Mie", "Jue", "Vie", "Sab", "Dom"].map((d, i) => (
                <div key={d} className="flex flex-col items-center justify-end">
                   ...
                   <div className="w-full bg-slate-100 rounded-full h-2 mb-3 relative overflow-hidden">
                      <div className="absolute top-0 left-0 bottom-0 bg-orange-500 rounded-full" style={{ width: freqUiByDay[i] > 0 ? '100%' : '5%' }}></div>
                   </div>
      ...
      {/* Table Detalle de tardanzas */}
      <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden flex flex-col">
        <div className="p-6 border-b border-slate-200 flex items-start justify-between">
          <div>
            <h2 className="text-xl font-black text-slate-900">Detalle de tardanzas</h2>
            <p className="text-[11px] font-semibold text-slate-500 mt-0.5">Registros ordenados desde la marcacion mas reciente.</p>
          </div>
          <div className="bg-orange-50 text-orange-700 text-xs font-bold px-3 py-1.5 rounded-full border border-orange-200">
            {filteredTardanzas.length} registro(s)
          </div>
        </div>
          ...
          <table className="w-full text-left text-[11px]">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 font-extrabold text-slate-500 uppercase tracking-widest text-center">Hora registrada</th>
              ...
              {filteredTardanzas.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-10 text-center text-slate-500 font-semibold text-sm">
                    No se encontraron registros de tardanza bajo estos filtros.
                  </td>
                </tr>
              ) : (
                filteredTardanzas.map((m, idx) => {
                  ...
                  <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                       <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black tracking-wide border bg-amber-50 text-amber-700 border-amber-200">
        ...
        <div className="p-4 border-t border-slate-200 flex justify-between items-center bg-white">
          <p className="text-[10px] font-semibold text-slate-500">Mostrando 1 - {filteredTardanzas.length} de {filteredTardanzas.length}</p>
